...
export default function MyGroups() {
    ...
    const startLiveSession = async (groupId, roomId,startTime,endTime) => {
        try {
            ...
            await axios.post(`${process.env.REACT_APP_API_BASE_URL}/groups/sessions`, {
                group_id: groupId,
                session_date: sessionDate,
                start_time: startTime,
                end_time: endTime,
                meeting_link: `${window.location.origin}/romm-meating/${roomId}`
            });
        } catch (error) {
    ...
    const LiveSchedule = ({ daily_start, daily_end }) => {
        ...
        useEffect(() => {
            const updateTime = () => {
                ...
                // Day label
                const startDay = germanDays[startDate.getDay()];
                const endDay = germanDays[endDate.getDay()];
                const startStr = startDate.toISOString().split("T")[0];
                const endStr = endDate.toISOString().split("T")[0];
                setDayLabel(`📅 ${startDay} . ${startStr} → ${endDay} . ${endStr}`);
            };
